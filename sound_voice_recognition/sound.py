...
class Sound:
    ...
    def __init__(self):
        ...
        self.lang = ['en', '']  # 0 - lang,  1 - last_lang
    ...
    def check_lang(self):
        ...
        ru = not GPIO.input(LANG_BTN_RU)
        ua = not GPIO.input(LANG_BTN_UA)
        en = not GPIO.input(LANG_BTN_EN)
        if ru:
            self.lang[0] = 'ru'
        if ua:
            self.lang[0] = 'ua'
        if not ru and not ua:
            self.lang[0] = 'en'

        if self.lang[0] != self.lang[1]:
            self.lang[1] = self.lang[0]
            print('ru = {0}, ua = {1}, en = {2}'.format(ru, ua, en))
            print('Lang was changed, now it\'s', self.lang[0])

    def pass_recognition(self):
        if self.sig == 8 and GPIO.input(SIG_7_IN):
            print('bypass 8 was pressed 0')
            return True
    ...
    def before_recognize(self):
        ...
        while GPIO.input(GERK):             # жду сработку геркона
            self.if_reset()                 # если нажат ресет
            self.if_help()                  # если нажата кнопка Help
            if self.pass_recognition():     # усли ушел sig 8
                return 1
                ...
                if r.correct_wrd != 1:
                    ...
                    if r.correct_wrd == 0:
                        ...
                        if self.pass_recognition():
                            print('bypass 8 was pressed 3')
                            r.pause_all()        # закончить распознавание, загадка решена
                            break
    ...
    def GPIOsetup(self):
        ...
        li_all_inp = SIG_IN + [LANG_BTN_EN, LANG_BTN_UA, LANG_BTN_RU, HELP_BTN, GERK]
        for pin in li_all_inp:
            if pin:
                GPIO.setup(pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)

    def if_reset(self):
        '''отслеживаю нажатие reset'''
        if not GPIO.input(SIG_1_IN):
            self.reset()
    
    def if_help(self):
        '''проверяю нажатие кнопки help и включаю подсказку'''
    ...
    def replay(self):
        ...
        # если replay_tm != 0 и REPLAY_TIME != 0, вкл повтор голоса
